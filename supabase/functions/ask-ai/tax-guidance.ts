...
export function lookupTaxGuidance(context: AskAiContext, categories: AskAiCategory[]): TaxGuidance | null {
  const availableCategories = new Set(categories.map((category) => category.name));
  ...
  if (/\b(personal|for home|family|owner draw|owner personal|not business|private use)\b/.test(useText)) {
  ...
  if (/\b(penalty|penalties|fine|fines|federal income tax|estimated income tax|irs payment|1040 payment)\b/.test(combinedText)) {
    return guidance(availableCategories, {
  ...
  if (/\b(resale|resell|finished goods?|inventory for sale|sell as is|sold as is|wholesale item)\b/.test(useText)) {
    return guidance(availableCategories, {
      id: "resale-inventory",
  ...
  if (isProductPackagingUseCase(combinedText, useText) &&
  ...
  if (/\b(freight in|inbound shipping|shipping from supplier|supplier shipping|shipping for ingredients|shipping for jars|freight charge)\b/.test(combinedText)) {
    return guidance(availableCategories, {
      id: "freight-in",
  ...
  if (/\b(shipping box|mailers?|bubble wrap|packing tape|tissue paper|void fill|packing paper|poly mailer|ship orders?|send orders?|customer shipment)\b/.test(combinedText)) {
    return guidance(availableCategories, {
  ...
  if (/\b(postage|shipping label|usps|ups|fedex|pirate ship|shipstation|shopify shipping|delivery to customer)\b/.test(combinedText)) {
    return guidance(availableCategories, {
  ...
  if (/\b(electric|electricity|water bill|gas utility|internet|wifi|phone service|utility|utilities)\b/.test(combinedText)) {
    return guidance(availableCategories, {
      id: "utilities",
  ...
  if (/\b(facebook ads?|google ads?|meta ads?|advertis|marketing|promo|promotion|sponsorship|booth sign|banner|flyer|business cards?|canva)\b/.test(combinedText)) {
    return guidance(availableCategories, {
      id: "advertising",
  ...
  if (/\b(shopify fee|square fee|stripe|paypal fee|merchant fee|processing fee|commission|faire commission|transaction fee)\b/.test(combinedText)) {
    return guidance(availableCategories, {
      id: "merchant-fees",
  ...
  if (/\b(liability insurance|business insurance|product liability|insurance premium)\b/.test(combinedText)) {
    return guidance(availableCategories, {
      id: "insurance",
  ...
  if (/\b(printer paper|receipt paper|pens?|file folders?|desk organizers?|office|admin|storage bins?|organizer|clipboard|staples?)\b/.test(combinedText) &&
  ...
  if (/\b(cpa|accountant|bookkeep|lawyer|attorney|legal|consultant|professional service|tax preparer)\b/.test(combinedText)) {
    return guidance(availableCategories, {
  ...
  if (/\b(business license|license fee|permit|state filing|sales tax permit|registration fee)\b/.test(combinedText)) {
    return guidance(availableCategories, {
      id: "taxes-licenses",
  ...
  if (/\b(interest charge|credit card interest|loan interest|business loan interest)\b/.test(combinedText)) {
    return guidance(availableCategories, {
      id: "interest-expense",
...
export function buildTaxGuidancePromptBlock(guidance: TaxGuidance | null): string {
  if (!guidance) return "";
...
export function applyTaxGuidance(result: AskAiResult, guidance: TaxGuidance | null, categories: AskAiCategory[]): AskAiResult {
...
function chooseAvailableCategory(availableCategories: Set<string>, candidates: string[]): string {
  return candidates.find((category) => availableCategories.has(category)) || "Needs Review";
}
...
function isFoodOrBeverage(text: string): boolean {
  ...
  return /\b(coffee|espresso|folgers|starbucks|k\s*cup|kcup|tea|cocoa|bottled water|water bottles?|soda|soft drinks?|drink|drinks|beverage|beverages|doughnuts?|donuts?|snacks?|food|lunch|meal|meals|breakfast|candy|cookies?)\b/.test(text);
}

function isWorkerRefreshmentUseCase(text: string): boolean {
  return /\b(employee|employees|staff|team|crew|worker|workers|volunteer|volunteers|intern|interns|we|us|our)\b/.test(text) &&
    /\b(work|working|shift|shifts|shop|office|studio|production|market|event|drink|drinks|eat|snack|breakroom|break room)\b/.test(text);
}
...
function isProductPackagingUseCase(combinedText: string, useText: string): boolean {
  if (/\b(product label|ingredient label|soap box|shrink wrap|retail packaging|packaging attached|goes with product)\b/.test(combinedText)) {
    return true;
  }
